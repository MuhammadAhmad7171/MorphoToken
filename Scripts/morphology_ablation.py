...
Morphological-operation ablation mode trains M0--M6 under one fixed data split and writes
paper-ready metrics, tables, confusion matrices, learning curves and summaries.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--stage", choices=("final", "verify-data", "verify-preprocess", "verify-model", "train", "external", "all", "gridsearch", "morphology-ablation"), default="final")
    ...
    p.add_argument("--split-seed", type=int, default=2026,
                   help="Fixed BRISC train/validation split seed; independent of model seed")
    p.add_argument("--architecture-variant", choices=("A0", "A1", "A2", "A3", "A4", "A5", "A6"), default="A6")
    p.add_argument("--morphology-variant", choices=("M0", "M1", "M2", "M3", "M4", "M5", "M6"), default="M3",
                   help="Morphology branch used by the full A6 architecture")
    p.add_argument("--morphology-ablation-seeds", type=str, default="2026,3407,5891",
                   help="Comma-separated training seeds used by --stage morphology-ablation")
    p.add_argument("--morphology-ablation-variants", type=str, default="M0,M1,M2,M3,M4,M5,M6",
                   help="Comma-separated morphology IDs used by --stage morphology-ablation")
    p.add_argument("--morphology-ablation-skip-completed", action=argparse.BooleanOptionalAction, default=True,
                   help="Reuse a run when brisc_final_metrics.json and best.pt already exist")
    ...
    try:
        args.morphology_ablation_seeds = tuple(int(x.strip()) for x in args.morphology_ablation_seeds.split(",") if x.strip())
    except ValueError:
        p.error("--morphology-ablation-seeds must contain comma-separated integers")
    args.morphology_ablation_variants = tuple(x.strip().upper() for x in args.morphology_ablation_variants.split(",") if x.strip())
    if not args.morphology_ablation_seeds:
        p.error("--morphology-ablation-seeds cannot be empty")
    if not args.morphology_ablation_variants or any(x not in {f"M{i}" for i in range(7)} for x in args.morphology_ablation_variants):
        p.error("--morphology-ablation-variants must contain IDs from M0 through M6")
...
class MorphologyFeatures(nn.Module):
    """Controlled morphology branch for M1--M6.

    M3 exactly reproduces the original implementation: identity features plus
    gradients at kernels 3/5/7. M6 removes the identity term inside this branch,
    making it a non-duplicate gradient-only operator comparison.
    """
    def __init__(self, channels: int, out_channels: int, variant: str):
        super().__init__()
        self.variant = variant
        settings = {
            "M1": ((3,), "gradient", True),
            "M2": ((3, 5), "gradient", True),
            "M3": ((3, 5, 7), "gradient", True),
            "M4": ((3, 5, 7), "dilation", True),
            "M5": ((3, 5, 7), "erosion", True),
            "M6": ((3, 5, 7), "gradient", False),
        }
        if variant not in settings:
            raise ValueError(f"MorphologyFeatures does not support {variant}")
        self.kernels, self.operation, self.include_identity = settings[variant]
        in_blocks = len(self.kernels) + int(self.include_identity)
        self.project = nn.Sequential(
            nn.Conv2d(channels * in_blocks, out_channels, 1, bias=False),
            nn.GroupNorm(8, out_channels), nn.GELU())

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        outputs = [x] if self.include_identity else []
        for k in self.kernels:
            dilation = F.max_pool2d(x, k, stride=1, padding=k // 2)
            erosion = -F.max_pool2d(-x, k, stride=1, padding=k // 2)
            if self.operation == "dilation": outputs.append(dilation)
            elif self.operation == "erosion": outputs.append(erosion)
            else: outputs.append(dilation - erosion)
...
class MorphoToken(nn.Module):
    def __init__(self, image_size: int, dim: int, depth: int, heads: int, dropout: float,
                 ...
                 variant: str = "A6", morphology_variant: str = "M3"):
        super().__init__()
        if image_size % 32:
        ...
        self.variant = variant
        self.morphology_variant = morphology_variant
        ...
        # M0 bypasses these branches in forward; M3 is the published/current full branch.
        branch_variant = "M3" if morphology_variant == "M0" else morphology_variant
        self.morph3 = MorphologyFeatures(dim, dim, branch_variant)
        self.morph4 = MorphologyFeatures(dim, dim, branch_variant)
        ...
        self.resnet_classifier = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(),
                                               nn.Dropout(dropout), nn.Linear(2048, self.num_classes))
        ...
        self._configure_variant_trainability()

    def _freeze_modules(self, *modules: nn.Module) -> None:
        for module in modules:
            for parameter in module.parameters():
                parameter.requires_grad_(False)

    def _configure_variant_trainability(self) -> None:
        """Exclude modules that do not participate in a variant's forward path."""
        if self.variant == "A0":
            self._freeze_modules(self.reduce3, self.reduce4, self.morph3, self.morph4,
                                 self.fuse3, self.fuse4, self.scale_fuse, self.encoder,
                                 self.norm, self.gate, self.cnn_classifier)
            self.pos.requires_grad_(False); self.prototypes.requires_grad_(False); self.fusion_logit.requires_grad_(False)
        elif self.variant == "A1":
            self._freeze_modules(self.reduce3, self.morph3, self.morph4, self.fuse3,
                                 self.fuse4, self.scale_fuse, self.gate, self.resnet_classifier)
            self.prototypes.requires_grad_(False); self.fusion_logit.requires_grad_(False)
        elif self.variant == "A2":
            self._freeze_modules(self.reduce3, self.morph3, self.fuse3, self.scale_fuse,
                                 self.resnet_classifier)
        elif self.variant == "A3":
            self._freeze_modules(self.backbone["layer4"], self.reduce4, self.morph4,
                                 self.fuse4, self.scale_fuse, self.resnet_classifier)
        elif self.variant == "A4":
            self._freeze_modules(self.morph3, self.morph4, self.fuse3, self.fuse4,
                                 self.resnet_classifier)
        else:
            self._freeze_modules(self.resnet_classifier)
            if self.variant == "A5":
                self._freeze_modules(self.encoder)
        if self.morphology_variant == "M0" and self.variant in {"A2", "A3", "A5", "A6"}:
            self._freeze_modules(self.morph3, self.morph4, self.fuse3, self.fuse4)

    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        stage2 = self.backbone["stem"](x)
        raw3 = self.backbone["layer3"](stage2)
        raw4 = None if self.variant == "A3" else self.backbone["layer4"](raw3)
        if self.variant == "A0":
            return {"logits": self.resnet_classifier(raw4)}
        local3 = self.reduce3(raw3) if self.variant != "A2" else None
        local4 = self.reduce4(raw4) if self.variant != "A3" else None
        if self.variant == "A1":
            feat = F.interpolate(local4, size=(self.side, self.side), mode="bilinear", align_corners=False)
            tokens = feat.flatten(2).transpose(1, 2) + self.pos
            tokens = self.norm(self.encoder(tokens))
            return {"logits": self.cnn_classifier(tokens.mean(dim=1))}

        # A2/A3 isolate one morphology-enriched scale; A4 isolates dual-scale
        # fusion without morphology; A5 removes contextual Transformer reasoning.
        no_morphology = self.variant == "A4" or self.morphology_variant == "M0"
        feat3 = (self.fuse3(torch.cat([local3, self.morph3(local3)], dim=1))
                 if local3 is not None and not no_morphology else None)
        feat4 = (self.fuse4(torch.cat([local4, self.morph4(local4)], dim=1))
                 if local4 is not None and not no_morphology else None)
        feat4_up = (F.interpolate(feat4, size=(self.side, self.side), mode="bilinear", align_corners=False)
                    if feat4 is not None else None)
        if self.variant == "A2":
            feat = feat4_up
        elif self.variant == "A3":
            feat = feat3
        elif self.variant == "A4" or self.morphology_variant == "M0":
            local4_up = F.interpolate(local4, size=local3.shape[-2:], mode="bilinear", align_corners=False)
            feat = self.scale_fuse(torch.cat([local3, local4_up], dim=1)) + 0.5 * (local3 + local4_up)
        else:
            feat = self.scale_fuse(torch.cat([feat3, feat4_up], dim=1)) + 0.5 * (feat3 + feat4_up)
        tokens = feat.flatten(2).transpose(1, 2) + self.pos
        tokens = self.norm(tokens if self.variant == "A5" else self.encoder(tokens))
...
def morphotoken_loss(out: Dict[str, torch.Tensor], labels: torch.Tensor, masks: torch.Tensor,
    ...
    if "proto_logits" not in out:
        zero = ce.new_zeros(())
        return ce, {"total": ce.item(), "ce": ce.item(), "branch": 0.0,
                    "mask": 0.0, "proto": 0.0, "sep": 0.0, "div": 0.0}
    branch_loss = 0.5 * (
...
def build_model(args: argparse.Namespace, pretrained: bool) -> MorphoToken:
    return MorphoToken(args.image_size, args.embed_dim, args.transformer_depth, args.heads,
                       ...
                       args.backbone, pretrained, args.architecture_variant,
                       args.morphology_variant)
...
def verify_model(args: argparse.Namespace, device: torch.device, rank: int, distributed: bool) -> None:
    ...
    loss.backward()
    if out["logits"].shape != (b, len(CLASSES)):
        raise RuntimeError(f"Unexpected logits shape: {tuple(out['logits'].shape)}")
    if "gate_map" in out and out["gate_map"].shape[-2:] != (args.image_size // 16,) * 2:
        raise RuntimeError(f"Unexpected gate-map shape: {tuple(out['gate_map'].shape)}")
...
def train(args: argparse.Namespace, train_samples: Sequence[Sample], test_samples: Sequence[Sample],
    ...
    train_ids, val_ids = stratified_split(train_samples, args.val_fraction, args.split_seed)
...
MORPHOLOGY_NAMES = {
    "M0": "No morphological branch",
    "M1": "Identity + gradient kernel 3",
    "M2": "Identity + gradient kernels 3 and 5",
    "M3": "Complete multi-scale morphology (identity + gradients 3/5/7)",
    "M4": "Identity + multi-scale dilation features",
    "M5": "Identity + multi-scale erosion features",
    "M6": "Multi-scale morphological gradients only",
}


@torch.no_grad()
def profile_ablation_model(args: argparse.Namespace, checkpoint: Path,
                           device: torch.device) -> Dict[str, object]:
    """Parameter, storage, optional FLOPs and forward-latency profile."""
    model = build_model(args, pretrained=False).to(device)
    load_model_checkpoint(checkpoint, model, device)
    model.eval()
    ...
    flops = None
    try:
        from thop import profile  # optional; the ablation remains runnable without it
    ...
    result = {
        "parameters": int(trainable), "trainable_parameters": int(trainable),
        "parameters_million": float(trainable / 1e6),
        "stored_parameters": int(total), "stored_parameters_million": float(total / 1e6),
        "checkpoint_megabytes": float(checkpoint.stat().st_size / (1024 ** 2)),
        "flops": flops, "gflops": (float(flops / 1e9) if flops is not None else None),
        "latency_ms_batch1_mean": float(np.mean(times)),
        "latency_ms_batch1_std": float(np.std(times, ddof=1)),
        "peak_cuda_memory_mb_batch1": peak_mb,
        "profile_tta": False,
    }
    del model, x
    if device.type == "cuda":
...
def _mean_sd_ci(values: Sequence[float]) -> Dict[str, float]:
    ...
    half = float(1.96 * sd / math.sqrt(len(a))) if len(a) > 1 else 0.0
    return {"mean": mean, "sd": sd, "ci95_low": mean - half,
            "ci95_high": mean + half, "n": int(len(a))}


def _flatten_ablation_row(variant: str, seed: int, metrics: Dict[str, object],
                          profile: Dict[str, object], run_dir: Path,
                          all_splits: Optional[Dict[str, object]] = None) -> Dict[str, object]:
    row: Dict[str, object] = {
        "variant": variant, "name": MORPHOLOGY_NAMES[variant], "seed": seed,
        "run_directory": str(run_dir), "accuracy": metrics["accuracy"],
        "macro_f1": metrics["macro_f1"], "balanced_accuracy": metrics["balanced_accuracy"],
        "macro_auroc": metrics["macro_auroc"], "nll": metrics["nll"],
        "brier": metrics["brier"], "ece_15bin": metrics["ece_15bin"],
    }
    for cls in CLASSES:
        for metric in ("precision", "recall", "f1"):
            row[f"{cls}_{metric}"] = metrics["per_class"][cls][metric]
        row[f"{cls}_support"] = metrics["per_class"][cls]["support"]
        row[f"{cls}_auroc"] = metrics["auroc_ovr"][cls]
    row.update(profile)
    if all_splits is not None:
        for split in ("train", "validation", "internal_test"):
            split_metrics = all_splits[split]
            for key in ("accuracy", "macro_f1", "balanced_accuracy", "macro_auroc",
                        "nll", "brier", "ece_15bin"):
                row[f"{split}_{key}"] = split_metrics[key]
            for cls in CLASSES:
                row[f"{split}_{cls}_f1"] = split_metrics["per_class"][cls]["f1"]
                row[f"{split}_{cls}_recall"] = split_metrics["per_class"][cls]["recall"]
    return row
...
def create_ablation_figures(root: Path, rows: Sequence[Dict[str, object]],
                            aggregate: Dict[str, object]) -> List[str]:
    """Create PNG/PDF plots; returns warnings instead of breaking experiments."""
    warnings: List[str] = []
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    variants = [v for v in MORPHOLOGY_NAMES if v in aggregate]
    labels = [f"{v}\n{MORPHOLOGY_NAMES[v]}" for v in variants]

    def save(fig, name):
        fig.tight_layout(); fig.savefig(fig_dir / f"{name}.png", dpi=300, bbox_inches="tight")
        fig.savefig(fig_dir / f"{name}.pdf", bbox_inches="tight"); plt.close(fig)

    # Main performance with across-seed error bars.
    fig, ax = plt.subplots(figsize=(11, 5.5)); x = np.arange(len(variants)); width = 0.25
    for j, (key, label) in enumerate((("accuracy", "Accuracy"), ("macro_f1", "Macro-F1"),
                                      ("balanced_accuracy", "Balanced accuracy"))):
        means = [aggregate[v][key]["mean"] * 100 for v in variants]
        errors = [aggregate[v][key]["sd"] * 100 for v in variants]
        ax.bar(x + (j - 1) * width, means, width, yerr=errors, capsize=3, label=label)
    ax.set_ylabel("Score (%)"); ax.set_xticks(x); ax.set_xticklabels(labels, fontsize=8)
    ax.set_title("Morphological-operation Ablation Performance (mean ± SD)"); ax.legend(ncol=3)
    ax.grid(axis="y", alpha=.25); save(fig, "morphology_performance")

    # Per-class F1.
    fig, ax = plt.subplots(figsize=(11, 5.5)); width = .18
    for j, cls in enumerate(CLASSES):
        means = [aggregate[v][f"{cls}_f1"]["mean"] * 100 for v in variants]
        ax.bar(x + (j - 1.5) * width, means, width, label=cls.replace("_", " "))
    ax.set_ylabel("F1 (%)"); ax.set_xticks(x); ax.set_xticklabels(labels, fontsize=8)
    ax.set_title("Class-wise F1 by Morphology Configuration"); ax.legend(ncol=4); ax.grid(axis="y", alpha=.25)
    save(fig, "classwise_f1")

    # Calibration and probabilistic quality.
    fig, axes = plt.subplots(1, 3, figsize=(12, 4))
    for ax, key, title in zip(axes, ("ece_15bin", "brier", "nll"),
                              ("ECE (lower is better)", "Brier score", "Negative log-likelihood")):
        vals = [aggregate[v][key]["mean"] for v in variants]
        ax.bar(variants, vals, color="#4472C4"); ax.set_title(title); ax.grid(axis="y", alpha=.25)
    save(fig, "calibration_metrics")

    # Complexity/performance trade-off.
    fig, ax = plt.subplots(figsize=(7, 5.5))
    for v in variants:
        p = aggregate[v]["parameters_million"]["mean"]
        f = aggregate[v]["macro_f1"]["mean"] * 100
        ax.scatter(p, f, s=80); ax.annotate(v, (p, f), xytext=(5, 5), textcoords="offset points")
    ax.set_xlabel("Parameters (million)"); ax.set_ylabel("Macro-F1 (%)")
    ax.set_title("Accuracy–Complexity Trade-off"); ax.grid(alpha=.25); save(fig, "complexity_tradeoff")

    # Confusion matrices for every seed-level run.
    for row in rows:
        metrics_path = Path(str(row["run_directory"])) / "brisc_internal_test_metrics.json"
        metrics = json.loads(metrics_path.read_text(encoding="utf-8")); cm = np.asarray(metrics["confusion_matrix"])
        fig, ax = plt.subplots(figsize=(5.2, 4.4)); im = ax.imshow(cm, cmap="Blues")
        for i in range(len(CLASSES)):
            for j in range(len(CLASSES)):
                ax.text(j, i, int(cm[i, j]), ha="center", va="center",
                        color="white" if cm[i, j] > cm.max() * .55 else "black")
        ax.set_xticks(range(4)); ax.set_yticks(range(4)); ax.set_xticklabels(CLASSES, rotation=30, ha="right")
        ax.set_yticklabels(CLASSES); ax.set_xlabel("Predicted"); ax.set_ylabel("True")
        ax.set_title(f"{row['variant']} seed {row['seed']}"); fig.colorbar(im, ax=ax, fraction=.046)
        save(fig, f"confusion_{row['variant']}_seed{row['seed']}")

    # Validation learning curves for all runs.
    fig, ax = plt.subplots(figsize=(10, 6))
    for row in rows:
        history = Path(str(row["run_directory"])) / "history.jsonl"
        records = [json.loads(line) for line in history.read_text(encoding="utf-8").splitlines() if line.strip()]
        ax.plot([r["epoch"] for r in records], [r["validation"]["macro_f1"] * 100 for r in records],
                alpha=.65, label=f"{row['variant']}-s{row['seed']}")
    ax.set_xlabel("Epoch"); ax.set_ylabel("Validation macro-F1 (%)"); ax.set_title("Ablation Learning Curves")
    ax.grid(alpha=.25); ax.legend(ncol=3, fontsize=7); save(fig, "validation_learning_curves")
    return warnings


def run_morphology_ablation(args: argparse.Namespace, train_samples: Sequence[Sample],
                              test_samples: Sequence[Sample], device: torch.device,
                              rank: int, local_rank: int) -> None:
    if rank != 0:
        raise RuntimeError("Morphological-operation ablation mode is single-GPU; launch with python, not torchrun")
    root = args.output_dir
    runs_root = root / "runs"; runs_root.mkdir(parents=True, exist_ok=True)
    rows: List[Dict[str, object]] = []
    total_runs = len(args.morphology_ablation_variants) * len(args.morphology_ablation_seeds); run_number = 0
    for variant in args.morphology_ablation_variants:
        for seed in args.morphology_ablation_seeds:
            run_number += 1
            run_dir = runs_root / variant / f"seed_{seed}"
            metrics_path = run_dir / "brisc_internal_test_metrics.json"
            checkpoint = run_dir / "best.pt"
            run_args = copy.deepcopy(args)
            run_args.stage = "train"; run_args.architecture_variant = "A6"
            run_args.morphology_variant = variant
            run_args.seed = seed; run_args.split_seed = args.split_seed
            run_args.output_dir = run_dir; run_args.resume = None
            run_dir.mkdir(parents=True, exist_ok=True)
            write_json(run_dir / "run_config.json", vars(run_args))
            print(f"[morphology-ablation {run_number:02d}/{total_runs:02d}] {variant} "
                  f"({MORPHOLOGY_NAMES[variant]}) seed={seed}", flush=True)
            if not (args.morphology_ablation_skip_completed and metrics_path.exists() and checkpoint.exists()):
                seed_everything(seed, rank)
                checkpoint = train(run_args, train_samples, test_samples, device,
                                   False, rank, local_rank, 1, finalize=True)
            else:
                print(f"[morphology-ablation] reusing completed run: {run_dir}", flush=True)
            metrics = json.loads(metrics_path.read_text(encoding="utf-8"))
            all_splits = json.loads((run_dir / "brisc_final_metrics.json").read_text(encoding="utf-8"))
            profile_path = run_dir / "model_profile.json"
            if profile_path.exists() and args.morphology_ablation_skip_completed:
                profile = json.loads(profile_path.read_text(encoding="utf-8"))
            else:
                profile = profile_ablation_model(run_args, checkpoint, device)
                write_json(profile_path, profile)
            rows.append(_flatten_ablation_row(variant, seed, metrics, profile, run_dir, all_splits))
            _write_csv(root / "morphology_ablation_seed_results.csv", rows)
            write_json(root / "morphology_ablation_seed_results.json", rows)
    ...
    for variant in args.morphology_ablation_variants:
        ...
        aggregate[variant] = {"name": MORPHOLOGY_NAMES[variant], "seeds": [r["seed"] for r in selected]}
        for key in numeric_keys:
            vals = [float(r[key]) for r in selected if r.get(key) is not None and math.isfinite(float(r[key]))]
            if vals: aggregate[variant][key] = _mean_sd_ci(vals)

    if "M3" in args.morphology_ablation_variants:
        full_by_seed = {int(r["seed"]): r for r in rows if r["variant"] == "M3"}
        comparisons = {}
        for variant in args.morphology_ablation_variants:
            if variant == "M3": continue
            variant_by_seed = {int(r["seed"]): r for r in rows if r["variant"] == variant}
            common = sorted(set(full_by_seed) & set(variant_by_seed))
            comparisons[variant] = {}
            for metric in ("accuracy", "macro_f1", "balanced_accuracy", "macro_auroc"):
                # Positive means the full MorphoToken outperformed the ablated model.
                differences = [float(full_by_seed[s][metric]) - float(variant_by_seed[s][metric]) for s in common]
                comparisons[variant][metric] = _mean_sd_ci(differences)
                comparisons[variant][metric]["full_model_wins"] = int(sum(d > 0 for d in differences))
        aggregate["paired_comparisons_vs_M3"] = comparisons

    aggregate_rows = []
    for variant in args.morphology_ablation_variants:
        row = {"variant": variant, "name": MORPHOLOGY_NAMES[variant], "n_seeds": len(args.morphology_ablation_seeds)}
        for key in numeric_keys:
            if key in aggregate[variant]:
                for stat in ("mean", "sd", "ci95_low", "ci95_high"):
                    row[f"{key}_{stat}"] = aggregate[variant][key][stat]
        aggregate_rows.append(row)
    _write_csv(root / "morphology_ablation_aggregate_results.csv", aggregate_rows)
    _write_csv(root / "paper_main_table_M0_M3.csv",
               [r for r in aggregate_rows if r["variant"] in {"M0", "M1", "M2", "M3"}])
    _write_csv(root / "supplementary_table_M4_M6.csv",
               [r for r in aggregate_rows if r["variant"] in {"M4", "M5", "M6"}])
    write_json(root / "morphology_ablation_aggregate_results.json", aggregate)
    warnings = create_ablation_figures(root, rows, aggregate)
    manifest = {
        "protocol": "M0-M6 morphological-operation ablation on fixed A6 architecture and BRISC split; model selection by validation macro-F1",
        "primary_table_variants": ["M0", "M1", "M2", "M3"],
        "supplementary_variants": ["M4", "M5", "M6"],
        "M3_M6_distinction": "M3 includes identity features inside the morphology branch; M6 contains gradients only",
        "split_seed": args.split_seed, "training_seeds": list(args.morphology_ablation_seeds),
        "variants": {v: MORPHOLOGY_NAMES[v] for v in args.morphology_ablation_variants},
        "external_datasets_used_for_selection": False,
        "primary_metric": "macro_f1", "error_bars": "across-seed standard deviation",
        "confidence_interval": "normal-approximation 95% CI across seeds; descriptive for n=3",
        "files": ["morphology_ablation_seed_results.csv", "morphology_ablation_seed_results.json",
                  "morphology_ablation_aggregate_results.csv", "morphology_ablation_aggregate_results.json",
                  "paper_main_table_M0_M3.csv", "supplementary_table_M4_M6.csv", "figures/"],
        "warnings": warnings,
    }
    write_json(root / "morphology_ablation_manifest.json", manifest)
    print(f"[morphology-ablation] COMPLETE: {root / 'morphology_ablation_aggregate_results.csv'}", flush=True)
...
def main() -> None:
    ...
    barrier(distributed)
    try:
        if args.stage == "morphology-ablation":
            if distributed:
                raise RuntimeError("Morphology ablation mode is configured for one GPU; launch with python, not torchrun")
            _, brisc_train, brisc_test, _ = verify_data(args, rank, False)
            run_morphology_ablation(args, brisc_train, brisc_test, device, rank, local_rank)
            return
        if args.stage == "final":
            if distributed:
